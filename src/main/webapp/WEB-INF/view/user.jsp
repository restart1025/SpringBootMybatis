...
<body>
    ...
    <img alt="" src="${pageContext.request.contextPath }/restart1025/logo.png">
    <script type="text/javascript" src="${pageContext.request.contextPath }/restart1025/jquery/jquery.min.js"></script>
    <script type="application/javascript">
        var userList = $.parseJSON('${userList}');
        $.each(userList, function(idx, obj) {
            $("#uldiv").append("<li>用户名：" + obj.username + "----密码：" + obj.password + "----<a href=\'${pageContext.request.contextPath }/user/edit/" + obj.id + "\'>修改用户（测试根据不同用户可访问权限不同，本例tom无权限，jack有权限）</a></li>");
        })
        <%--for( var i in userList ){--%>
            <%--$("#uldiv").append("<li>用户名：" + userList[i].username + "----密码：" + userList[i].password + "----<a href=\'${pageContext.request.contextPath }/user/edit/" + userList[i].id + "\'>修改用户（测试根据不同用户可访问权限不同，本例tom无权限，jack有权限）</a></li>");--%>
        <%--}--%>
    </script>
</body>
</html>